...
private:
    ...
    std::unordered_map<uint64_t, std::array<std::array<double, 2>, 10> > cache_read_data_to_all_2;
...
public:
    ...
    void NoCacheCalculateDes(int a, double &product_prob_given_ancestor, double &summary_stat_diff_ancestor);


    void CacheLoopDesAll(int index, int a, double &product_prob_given_ancestor, double &summary_stat_diff_ancestor);

    void CalculateLikelihoodAll(std::vector<SequenceProb> &all);
